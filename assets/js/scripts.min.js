...
jQuery(function() {
    ...
    jQuery('.nav-main > ul').append('<li class="magic-line menu-line hide-small"></li>');

});

jQuery.prototype.animateMenuLine = function(object,duration)
{console.log(jQuery(object));
  var width = (jQuery(object).actual('width') - 70) / 2;
  jQuery('.menu-line').stop().animate({
    left:     parseInt(jQuery(object).position().left + width + 10),
  }, {
    duration: duration,
    queue:    false,
    complete: function() { }
...
jQuery(window).load(function(){
  console.log(jQuery('.nav-main > ul > li.active'));

  jQuery(this).animateMenuLine(jQuery('.nav-main > ul > li.active'),0);

  jQuery('.nav-main > ul > li').hover(function() {
    jQuery(this).animateMenuLine(jQuery(this),100);

    /*TO DO*/
    var leftMax = (jQuery(this).width() - jQuery(this).find('.dropdown-menu').width()) / 2;
    jQuery(this).find('.dropdown-menu').css('margin-left', Math.min(leftMax - 15, 0));
  });

  jQuery('.nav-main > ul').hover(function() {  }, function() {
    jQuery(this).animateMenuLine(jQuery(this).children('li.active'),100);
  });
...
function lazyload() {
  ...
  jQuery('.fluid-span9').each(function(i, el){
    var ot = jQuery(el).offset().top;  //* top of object (i.e. advertising div)
    var ob = ot + jQuery(el).height(); //* bottom of object
    if (!jQuery(el).hasClass("animate") && wt <= ob && wb >= ot) {console.log('in');
      setTimeout( function() { jQuery(el).addClass('animate'); }, 200*i );
    }
  });
}
